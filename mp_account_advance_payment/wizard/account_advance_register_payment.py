...
class AccountAdvancePaymentRegister(models.TransientModel):
    ...
    ], string='Tipo de Pago', copy=False, default='outbound')
    ...
    def default_get(self, fields_list):
        ...
        for advance in advance_line_ids:
            ...
            """
                ...
                if account_id != advance.account_advance_id.id:
                    raise UserError("Los proveedores de los anticipos seleccionados deben tener la misma cuenta de anticipos para poder generar un único pago")
                
                if currency_id != advance.currency_id.id:
                    raise UserError("La moneda debe ser la misma para todos los anticipos")
            """
        ...
        _logger.error('journals_ids: %s' % (journal_ids))
        #res.update({'journal_id': []})
    ...
    def _create_move_vals_from_wizard(self,
        ...
        line_vals = []
        print("self.env.company.currency_id.id",self.env.company.currency_id.id)
        ...
        line_vals.append((0, 0, {
            ...
            'reference_one': new_payment.move_id.name
        }))
    ...
    def _create_payments_group(self):
        ...
        for advance in self.advance_ids:
            ...
            if not partner_ctl:    
                ...
                _logger.error('escribiendo el primer advance ctl %s' % (advance_ctl))
                ...
                if len(self.advance_ids) == 1:
                    ...
                    for move_line in new_payment.move_id.move_line_ids:
                        move_line.write({
                            'reference_one': move_line.move_id.name
                        })
                    advance_id.write({
    ...
    def _create_payments(self):
        ...
        for advance in self.advance_ids:
            ...
            new_payment.action_post()
            for move_line in new_payment.move_id.line_ids:
                move_line.write({
                    'reference_one': move_line.move_id.name
                })
            advance_id.write({
